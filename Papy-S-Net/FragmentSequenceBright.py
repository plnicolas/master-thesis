...
class FragmentSequenceBright(keras.utils.Sequence):
    """
    ...
    def __getitem__(self, index):
        ...
        while i < len(batchInputReturned):
            ...
            ############
            # Data augmentation: brightness and flipping
            ############

            randomNumber = np.random.random()
            randomNumber2 = np.random.random()

            if randomNumber2 < 0.5:
                #Brightness shift for one of the two images
                brightnessRNG = np.random.random()
                picture1 = imageGenerator.apply_transform(picture1, {"brightness" : brightnessRNG})
