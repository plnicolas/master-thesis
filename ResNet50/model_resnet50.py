...
def create_neural_network(widthImage, heightImage, initialLearningRate):
    ...
    # ResNet50
    model.add(keras.applications.resnet50.ResNet50(include_top=False,
                                                   weights='imagenet', input_shape=(heightImage, widthImage, 3), pooling='avg'))

    # Siamese network; two input images
    model1 = model(a)
    model2 = model(b)

    # Use the euclidean distance as the similarity measure between the two
    # fragments' feature maps
    distance = keras.layers.Lambda(
        euclidean_distance, output_shape=eucl_dist_output_shape)([model1, model2])
...
def train_network(model, learningSetGenerator, validationSetGenerator, numberEpochs, batchSize, initialLearningRate, maxQueueSize, numberEpochsLearningRate, discountFactor, prefixResults, stringInformation):
    ...
    tensorboardLogger = keras.callbacks.TensorBoard(log_dir="{}/tensorboard_log".format(
        pathResults), histogram_freq=0, batch_size=batchSize, write_grads=False, write_images=True, update_freq="epoch")

    csvLogger = keras.callbacks.CSVLogger(
        "{}/csv_log.csv".format(pathResults), separator=",")
    ...
    model.fit_generator(learningSetGenerator, epochs=numberEpochs, callbacks=[
                        tensorboardLogger, csvLogger, reduceLR], validation_data=validationSetGenerator, max_queue_size=maxQueueSize, workers=multiprocessing.cpu_count(), use_multiprocessing=True, verbose=2)
...
if __name__ == "__main__":
    ...
    PREFIX_RESULTS = "/home/plnicolas/codes/Results/ResNet50/BaseIN/"
