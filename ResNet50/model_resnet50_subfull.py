...
def train_network(model, learningSetGenerator, validationSetGenerator, numberEpochs, batchSize, initialLearningRate, maxQueueSize, numberEpochsLearningRate, discountFactor, prefixResults, stringInformation):
    ...
    #reduceLR = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, verbose=1, min_lr=0.0000001)
    
    model.fit_generator(learningSetGenerator, epochs=numberEpochs, callbacks=[tensorboardLogger, csvLogger, learningRateScheduler], validation_data=validationSetGenerator, max_queue_size=maxQueueSize, workers=multiprocessing.cpu_count(), use_multiprocessing=True, verbose=2)
...
if __name__ == "__main__":
    ...
    PREFIX_RESULTS = "/home/plnicolas/codes/Results/ResNet50/FullIN/"
    ADDITIONAL_INFORMATION = "ResNet50 with ImageNet weights and subfull. All weights are directly trainable. The loss function is the categorical cross-entropy. The optimizer is Adam with the default beta1 and beta2 parameters."
